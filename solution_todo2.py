...
class SWAGInference(object):
    ...
    def __init__(
        ...
        #self.max_num_models = self.swag_epochs/self.swag_update_freq
    ...
    def fit_swag(self, loader: torch.utils.data.DataLoader) -> None:
        ...
        self._swag_diagonal_samples = {name: collections.deque([param.clone()-self._swag_diagonal_mean[name]]) for name, param in self.network.named_parameters()}
    ...
    def calibrate(self, validation_data: torch.utils.data.Dataset) -> None:
        ...
        self._prediction_threshold = 2.0 / 3.0  # Initial threshold
        '''
        best_validation_acc = 0.0  # Initialize with a large value
        for threshold in range(0, 201, 50):  # Example threshold range (0 to 1 in steps of 0.05)
            threshold /= 100.0  # Convert to float
            
            # Make predictions on the validation set using the current threshold
            xs, is_snow, is_cloud, ys = validation_data.tensors
            pred_prob_all = self.network(xs)
            pred_ys = self.predict_labels(pred_prob_all)
            validation_acc = torch.mean((pred_ys == ys).float()).item()
            
            # Check if the current threshold results in lower loss
    ...
    def sample_parameters(self) -> None:
        ...
        for name, param in self.network.named_parameters():
            ...
            # Diagonal part
            sampled_param = current_mean + torch.div(current_std * z_1,(torch.ones_like(current_mean)*torch.tensor(1/torch.sqrt(torch.tensor(2))))) 

            # Full SWAG part
            if self.inference_mode == InferenceMode.SWAG_FULL:
                # TODO(2): Sample parameter values for full SWAG
                #raise NotImplementedError("Sample parameter for full SWAG")
                z_2 = torch.randn(self._swag_diagonal_samples[name][0].size(), requires_grad=False)
                sampled_param += torch.div(self._swag_diagonal_samples[name][0]* z_2,(torch.ones_like(current_mean)*torch.tensor(1/torch.sqrt(torch.tensor(2))))*torch.tensor(self.deviation_matrix_max_rank-1)) 
